...
/**
 * Excel配置转换器 - JavaScript版本
 */
class ExcelToJsonConverter {
  constructor(excelPath = 'excel', jsonPath = 'json') {
    ...
    try {
      ...
      workbook.SheetNames.forEach(sheetName => {
        try {
          ...
          // 第一行包含布局信息
          const layoutInfo = rawData[0];
          const positions = rawData.slice(1).map((row) => ({
  ...
  createLayoutsTemplate() {
    ...
    XLSX.writeFile(workbook, path.join(this.excelPath, 'layouts.xlsx'));
    console.log('📝 布局模板已创建');
  }
  ...
  writeExcelFile(name, data) {
    ...
    XLSX.writeFile(workbook, path.join(this.excelPath, `${name}.xlsx`));
  }
